...
site-studio/
├── packages/
│   ├── backend/          # Express + Codex Agent SDK
│   │   ├── src/
...
- Uses standard Codex tools: `Edit`, `Write`, `Glob`, `Grep`
...
This is what makes Site Studio unusual. The agent uses Codex Agent SDK's `canUseTool` callback:
...
- Codex Agent SDK (`@anthropic-ai/Codex-agent-sdk`)
